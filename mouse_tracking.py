...
test_uvd = []
for i in test_inds:
	print filepath + ("test_seq/%05i.png" % (i+1))
	depth = np.array(PIL.Image.open(filepath + ("test_seq/%05i.png" % (i+1))))
	xyz = np.loadtxt(filepath + ("test_seq/%05i.txt" % (i+1)))
	uvd = xyz2uvd(xyz)
...
def compute_deformation(interpolator, deformation_coords):
	...
	if simplex_num != -1:
		...
		if len(np.unique(simplex.round(4), axis=0)) == 1:
			return train_uvd_flattened[simplex_indices[0]].reshape(-1,3)[:,0:2].T
